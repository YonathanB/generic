...
function _getData(dataToGet, idx) {
    if (dataToGet.length > 0) {
        ...
        dataToGet.forEach(function(command){
            _sendToDevice(command);
        });
        return promises;
...
function _init() {
    ...
    _COMMUNICATION_OBJECT.onOpen(function () {
        ...
        _commandsResolver([deviceCommands.MODEL, deviceCommands.NAME], communicationReady);
...
(function () {
    ...
    //  */
        ...
        .factory('DataProxy', [
            ...
            '$q',
            '$interval',
            function ($communication, $translator, $timeout, $q, $interval) {
                var _self = this;
